 ...
 *         2018年1月24日
 */
public class CDLXhandlerForTS implements Handler {
	private static final Logger LOGGER = LoggerFactory.getLogger(CDLXhandlerForTS.class);
	...
	private static int NVRGBID = 10000;
	private static int CAMERAGBID = 10000;
	...
	static {
		...
		businessMap.put("商铺医药", "药店");
		businessMap.put("商铺  医药", "药店");
		businessMap.put("商铺 医药", "药店");
	...
	private String getNextGbIdForNVR() {
		return "130102000011800" + NVRGBID++;
	}

	private String getNextGbIdForCAMERA() {
		return "130102000013100" + CAMERAGBID++;
	}
	...
	private void handleNVR(Map userBasic) {
		...
		// ownerId: 90000+UserBasic.Unumber后四位
		String ownerId = "90000"
				+ (unumber.length() > 4 ? unumber.substring(unumber.length() - 4, unumber.length()) : unumber);
